...
typedef struct D3D11ComputeBuffer /* Cast FNA3D_Buffer* to this! */
{
	ID3D11Buffer* handle;
...
typedef struct D3D11Effect_New /* Cast FNA3D_Effect* to this! */
{
	ID3DX11Effect* effect;
} D3D11Effect_New;

typedef struct D3D11_Effect_Variable /* Cast FNA3D_Effect_Variable* to this! */
{
	ID3DX11EffectVariable* effect;
} D3D11_Effect_Variable;

typedef struct D3D11ComputeShader /* Cast FNA3D_Effect* to this! */
{
...
typedef struct D3D11Renderer /* Cast FNA3D_Renderer* to this! */
{
	...
	/* Compute */
	ID3D11Buffer* computeBuffers[MAX_BOUND_COMPUTE_BUFFERS];
	ID3D11UnorderedAccessView* CSUnorderedAccessViews[MAX_BOUND_COMPUTE_BUFFERS];
	...
	/* FX11 Interop */
	ID3DX11Effect* fx11CurrentEffect;
	ID3DX11EffectPass* fx11CurrentPass;
} D3D11Renderer;

static D3D11_BUFFER_UAV_FLAG XNAToD3D_ComputeBufferType[] = 
{
...
//		hr = D3DCompileFromFile(hlslFileName, NULL, D3D_COMPILE_STANDARD_FILE_INCLUDE, entryPoint, shaderModel,
//			dwShaderFlags, 0, ppBlobOut, &errorBlob);
//		if (FAILED(hr))
//		{
//			if (errorBlob != NULL)
//			{
//				FNA3D_LogInfo("Error: (%s)", (const char*)(errorBlob->lpVtbl->GetBufferPointer(errorBlob)));
//				OutputDebugStringA((const char*)(errorBlob->lpVtbl->GetBufferPointer(errorBlob)));
//			}
...
//		HRESULT hr = S_OK;
//		ID3DBlob* errorBlob = NULL;
//		hr = D3DCompile(src, size, sourceName, NULL, D3D_COMPILE_STANDARD_FILE_INCLUDE, entryPoint, shaderModel,
//			dwShaderFlags, 0, ppBlobOut, &errorBlob);
//		if (FAILED(hr))
//		{
//			if (errorBlob != NULL)
...
static void D3D11_INTERNAL_LogError(
	...
	{
		res = device->GetDeviceRemovedReason();// ID3D11Device_GetDeviceRemovedReason(device);
	}
...
static ID3D11BlendState* D3D11_INTERNAL_FetchBlendState(
	...
	/* Bake the state! */
	res = renderer->device->CreateBlendState(
		&desc,
		&result
	);
...
static ID3D11DepthStencilState* D3D11_INTERNAL_FetchDepthStencilState(
	...
	/* Bake the state! */
	res = renderer->device->CreateDepthStencilState(
		&desc,
		&result
	);
...
static ID3D11InputLayout* D3D11_INTERNAL_FetchBindingsInputLayout(
	...
	for (i = 0; i < numBindings; i += 1)
	{
		...
		for (j = 0; j < binding->vertexDeclaration.elementCount; j += 1)
		{
			...
			// If we are in FX11 context
			if (renderer->fx11CurrentPass != nullptr) 
			{
				attribLoc = numElements;
			}
			else 
			{
	...
	FNA3D_LogInfo("Num Elements: %d\n", numElements);
	for (int i = 0; i < numElements; i++)
	{
		printf("Element#%d: %s:%d, %d\n", i, elements[i].SemanticName, elements[i].SemanticIndex, elements[i].AlignedByteOffset);
	}
	...
	if (renderer->fx11CurrentPass != nullptr)
	{
		...
		renderer->fx11CurrentPass->GetDesc(&passDesc);
...
static void D3D11_Dispatch(
	...
	/* Dispatch compute task to threads */
	ID3D11DeviceContext_Dispatch(renderer->context, 
...
static FNA3D_Texture* D3D11_CreateTexture2D(
	...
	if (isRandomAccess) 
	{
		...
		ERROR_CHECK_RETURN("Texture2D unordered access view creation failed", NULL)
	}
...
static FNA3D_Texture* D3D11_CreateTexture3D(
	...
	if (isRandomAccess)
	{
		...
		ERROR_CHECK_RETURN("Texture3D unordered access view creation failed", NULL)
	}
...
static void D3D11_AddDisposeTexture(
	...
	if (tex->UAVView) 
	{
		ID3D11UnorderedAccessView_Release(tex->UAVView);
	}
...
static FNA3D_ComputeBuffer* D3D11_GenComputeBuffer(
	FNA3D_Renderer* driverData,
	uint8_t dynamic,
	...
	D3D11Renderer* renderer = (D3D11Renderer*)driverData;
	D3D11ComputeBuffer* result = (D3D11ComputeBuffer*)SDL_malloc(sizeof(D3D11ComputeBuffer));
	D3D11_BUFFER_DESC desc;
	HRESULT res;

	/* Initialize the descriptor */
	desc.ByteWidth = elementCount * strideSize;
	desc.Usage = dynamic ? D3D11_USAGE_DYNAMIC : D3D11_USAGE_DEFAULT;
	desc.BindFlags = D3D11_BIND_UNORDERED_ACCESS | D3D11_BIND_SHADER_RESOURCE;
	desc.CPUAccessFlags = dynamic ? D3D11_CPU_ACCESS_WRITE : 0;
	desc.MiscFlags = D3D11_RESOURCE_MISC_BUFFER_STRUCTURED;
	desc.StructureByteStride = strideSize;
	...
	ERROR_CHECK_RETURN("Compute buffer creation failed", NULL)
	...
	ERROR_CHECK_RETURN("Compute shader view creation failed", NULL)

	UINT flags = 0;

	if (type & FNA3D_COMPUTEBUFFER_TYPE_RAW) 
	{
		flags |= D3D11_BUFFER_UAV_FLAG_RAW;
	}

	if (type & FNA3D_COMPUTEBUFFER_TYPE_APPEND)
	{
		flags |= D3D11_BUFFER_UAV_FLAG_APPEND;
	}

	if (type & FNA3D_COMPUTEBUFFER_TYPE_COUNTER)
	{
	...
	res = ID3D11Device_CreateUnorderedAccessView(renderer->device,
			(ID3D11Resource*)result->handle,
			&uavDesc,
			&result->uav);

	ERROR_CHECK_RETURN("Compute unordered access view creation failed", NULL)
...
static void D3D11_AddDisposeComputeBuffer(
	FNA3D_Renderer* driverData,
	FNA3D_ComputeBuffer* buffer
) {
	D3D11Renderer* renderer = (D3D11Renderer*)driverData;
	D3D11ComputeBuffer* d3dComputeBuffer = (D3D11ComputeBuffer*)buffer;
	ID3D11Buffer* nullComputeBuffer[] = { NULL };
	ID3D11ShaderResourceView* nullSRV[] = { NULL };
	ID3D11UnorderedAccessView* nullUAV[] = { NULL };
	uint32_t whatever[1] = { 0 };
	int32_t i;

	for (i = 0; i < MAX_BOUND_COMPUTE_BUFFERS; i += 1)
	{
		if (renderer->computeBuffers[i] == d3dComputeBuffer->handle)
		{
			renderer->computeBuffers[i] = NULL;
			renderer->CSUnorderedAccessViews[i] = NULL;
	...
	ID3D11Buffer_Release(d3dComputeBuffer->handle);
	ID3D11ShaderResourceView_Release(d3dComputeBuffer->srv);
	ID3D11UnorderedAccessView_Release(d3dComputeBuffer->uav);
...
static void D3D11_CreateNewEffect(
	FNA3D_Renderer* driverData,
	...
	D3D11Renderer* renderer = (D3D11Renderer*)driverData;
	D3D11Effect_New* result;
	HRESULT res;
	
	result = (D3D11Effect_New*)SDL_malloc(sizeof(D3D11Effect_New));
	*effect = (FNA3D_Effect_New*)result;

	ID3DX11Effect* effectPtr;
	res = D3DX11CreateEffectFromMemory(effectBinaryCode, effectCodeLength, 0, renderer->device, &effectPtr);
	//res = ID3D11Device_CreateComputeShader(renderer->device, shaderBlob->lpVtbl->GetBufferPointer(shaderBlob),
	//	shaderBlob->lpVtbl->GetBufferSize(shaderBlob), NULL, &result->shader);
	ERROR_CHECK_RETURN("New effect create failed", )

	result->effect = effectPtr;
}


//static void D3D11_SetComputeBuffer(
//	FNA3D_Renderer* driverData,
//	uint32_t start,
//	uint32_t numBuffers,
...
//) {
//	D3D11Renderer* renderer = (D3D11Renderer*)driverData;
//	D3D11Buffer** d3dBuffers = (FNA3D_Buffer**)buffers;
//	HRESULT res;
//
//	ID3D11Buffer* handles[8];
//	for (int i = 0; i < 8 && i < numBuffers; i++)
//	{
//		handles[i] = d3dBuffers[i]->handle;
//	}
//
//	SDL_LockMutex(renderer->ctxLock);
...
//static void D3D11_SetCSConstantBuffer(
//	FNA3D_Renderer* driverData,
//	uint32_t start,
//	uint32_t numBuffers,
//	FNA3D_Buffer** buffers
...
static void D3D11_ApplyComputeShader(
	FNA3D_Renderer* driverData,
	FNA3D_ComputeShader* shader
) {
	D3D11Renderer* renderer = (D3D11Renderer*)driverData;

	D3D11ComputeShader* computeShader = (D3D11ComputeShader*)shader;
	SDL_LockMutex(renderer->ctxLock);
	ID3D11DeviceContext_CSSetShader(renderer->context, computeShader->shader, NULL, 0);
	SDL_UnlockMutex(renderer->ctxLock);
}

//static FNA3D_Texture* D3D11_CreateTexture2DUAV(
//	FNA3D_Renderer* driverData,
//	FNA3D_SurfaceFormat format,
//	int32_t width,
//	int32_t height,
...
//static void D3D11_CompileShaderNative(
//	FNA3D_Renderer* driverData,
//	const char* effectTextCode,
//	uint32_t effectCodeLength,
...
//	D3D11Renderer* renderer = (D3D11Renderer*)driverData;
//	HRESULT res;
//	res = CreateShaderFromMemory(effectTextCode, effectCodeLength, NULL, 
//		entryPoint, shaderModel, compiledShader);
//	ERROR_CHECK_RETURN("Failed to compile native shader", )
//}

//static void D3D11_CreateEffect11(
//	FNA3D_Renderer* driverData,
//	uint8_t* effectCode,
//	uint32_t effectCodeLength,
//	FNA3D_Effect** effect
...
//	D3D11Renderer* renderer = (D3D11Renderer*)driverData;
//
//	ID3DX11Effect* dx11_effect;
//	res = D3DX11CreateEffectFromMemory(effectCode, effectCodeLength, 0,
//		renderer->device, &dx11_effect, NULL);
//	D3DCompile
//	ERROR_CHECK_RETURN("Create Effect 11 Failed", NULL)
//
//	D3DX11Effect* result;
//
//	result = (D3DX11Effect*)SDL_malloc(sizeof(D3DX11Effect));
//	result->effect = dx11_effect;
...
static void D3D11_ApplyEffect(
	...
	renderer->currentTechnique = technique;
	renderer->currentPass = pass;
	// Reset fx11 effect, switch to MOJOSHADER context
	renderer->fx11CurrentEffect = nullptr;
	renderer->fx11CurrentPass = nullptr;
}
...
try_create_device:
	...
	renderer->swapchainDatas = (D3D11SwapchainData**)SDL_malloc(renderer->swapchainDataCapacity * sizeof(D3D11SwapchainData*));
...
static void D3D11_PLATFORM_CreateSwapChain(
	...
	{
		renderer->swapchainDataCapacity *= 2;
		renderer->swapchainDatas = (D3D11SwapchainData**) SDL_realloc(
			renderer->swapchainDatas,
			renderer->swapchainDataCapacity * sizeof(D3D11SwapchainData*)
		);
	}
...
{
	D3D11Effect_New* effectNew = (D3D11Effect_New*)effect;
	ID3DX11Effect* d3dEffect = effectNew->effect;
	return d3dEffect->GetVariableByName(name);
}

FX11API ID3DX11EffectVariable* FX11_Effect_GetVariableByIndex(FNA3D_Effect_New* effect, uint32_t index)
{
	D3D11Effect_New* effectNew = (D3D11Effect_New*)effect;
	ID3DX11Effect* d3dEffect = effectNew->effect;
	return d3dEffect->GetVariableByIndex(index);
}

FX11API ID3DX11EffectVariable* FX11_Effect_GetVariableBySemantic(FNA3D_Effect_New* effect, const char* semantic)
{
	D3D11Effect_New* effectNew = (D3D11Effect_New*)effect;
	ID3DX11Effect* d3dEffect = effectNew->effect;
	return d3dEffect->GetVariableBySemantic(semantic);
}

FX11API ID3DX11EffectTechnique* FX11_Effect_GetTechniqueByIndex(FNA3D_Effect_New* effect, uint32_t index)
{
	D3D11Effect_New* effectNew = (D3D11Effect_New*)effect;
	ID3DX11Effect* d3dEffect = effectNew->effect;
	return d3dEffect->GetTechniqueByIndex(index);
}

FX11API ID3DX11EffectTechnique* FX11_Effect_GetTechniqueByName(FNA3D_Effect_New* effect, const char* name)
{
	D3D11Effect_New* effectNew = (D3D11Effect_New*)effect;
	ID3DX11Effect* d3dEffect = effectNew->effect;
...
FX11API ID3DX11EffectPass* FX11_Effect_Technique_GetPassByIndex(ID3DX11EffectTechnique* technique, uint32_t index)
{
	return technique->GetPassByIndex(index);
}

FX11API ID3DX11EffectPass* FX11_Effect_Technique_GetPassByName(ID3DX11EffectTechnique* technique, const char* name)
{
	return technique->GetPassByName(name);
}

FX11API void FX11_Effect_Pass_Apply(FNA3D_Device* device, ID3DX11EffectPass* pass)
{
	D3D11Renderer* renderer = (D3D11Renderer*)device->driverData;
	HRESULT res;

	SDL_LockMutex(renderer->ctxLock);
	res = pass->Apply(0, renderer->context);
	SDL_UnlockMutex(renderer->ctxLock);

	if (FAILED(res))
	{
		D3D11_INTERNAL_LogError(renderer->device, "Failed to apply effect", res);
		return;
	}

	// This is an indication of current FX11 context
	renderer->fx11CurrentPass = pass;
	renderer->currentTechnique = nullptr;
	renderer->currentPass = -1;
}
...
{
	D3D11ComputeBuffer* d3dComputeBuffer = (D3D11ComputeBuffer*)computeBuffer;
	ID3D11UnorderedAccessView* unorderedAccessView = d3dComputeBuffer ? d3dComputeBuffer->uav : nullptr;

	static LPCSTR pFuncName = "FNA3D: FX11_Effect_Variable_SetUnorderedAccessView_ComputeBuffer";

	ID3DX11EffectUnorderedAccessViewVariable* UAVVar = variable->AsUnorderedAccessView();
	HRESULT res;

	res = UAVVar->SetUnorderedAccessView(unorderedAccessView);
	if (FAILED(res))
	{
		D3D11_INTERNAL_LogError(nullptr, "Failed to set Unordered Access View", res);
		// FNA3D_LogInfo("Error: (%s)", "Failed to set Unordered Access View");
		return;
	}
}

FX11API void FX11_Effect_Variable_SetUnorderedAccessView_Texture(ID3DX11EffectVariable* variable,
	FNA3D_Texture* texture)
{
	D3D11Texture* d3dTexture = (D3D11Texture*)texture;
	ID3D11UnorderedAccessView* unorderedAccessView = d3dTexture ? d3dTexture->UAVView : nullptr;

	static LPCSTR pFuncName = "FNA3D: FX11_Effect_Variable_SetUnorderedAccessView_Texture";
...
{
	D3D11ComputeBuffer* d3dComputeBuffer = (D3D11ComputeBuffer*)computeBuffer;
	ID3D11ShaderResourceView* shaderResourceView = d3dComputeBuffer ? d3dComputeBuffer->srv : nullptr;

	static LPCSTR pFuncName = "FNA3D: FX11_Effect_Variable_SetUnorderedAccessView_Texture";

	ID3DX11EffectShaderResourceVariable* SRVVar = variable->AsShaderResource();
	HRESULT res;

	res = SRVVar->SetResource(shaderResourceView);

	if (FAILED(res))
	{
		D3D11_INTERNAL_LogError(nullptr, "Failed to set Shader Resource View", res);
		// FNA3D_LogError("Error: (%s)", "Failed to set Shader Resource View");
		return;
	}
}

FX11API void FX11_Effect_Variable_SetShaderResourceView_Texture(ID3DX11EffectVariable* variable, FNA3D_Texture* texture)
{
	D3D11Texture* d3dTexture = (D3D11Texture*)texture;
	ID3D11ShaderResourceView* shaderResourceView = d3dTexture ? d3dTexture->shaderView : nullptr;

	static LPCSTR pFuncName = "FNA3D: FX11_Effect_Variable_SetShaderResourceView_Texture";
...
FX11API void FX11_Effect_Variable_SetInt(ID3DX11EffectVariable* variable, int value)
{
	ID3DX11EffectScalarVariable* scalarVar = variable->AsScalar();
	HRESULT res;
	res = scalarVar->SetInt(value);
	if (FAILED(res))
	{
		D3D11_INTERNAL_LogError(nullptr, "Failed to set Integer", res);
		// FNA3D_LogInfo("Error: (%s)", "Failed to set Integer");
		return;
	}
}

FX11API void FX11_Effect_Variable_SetVectorf(ID3DX11EffectVariable* variable, const float* valueArray)
{
	ID3DX11EffectVectorVariable* vectorVar = variable->AsVector();
	HRESULT res;

	res = vectorVar->SetFloatVector(valueArray);
	if (FAILED(res))
	{
		D3D11_INTERNAL_LogError(nullptr, "Failed to set Float Vector", res);
		return;
	}
}

FX11API void FX11_Effect_Variable_SetVectorb(ID3DX11EffectVariable* variable, const bool* valueArray)
{
	ID3DX11EffectVectorVariable* vectorVar = variable->AsVector();
	HRESULT res;

	res = vectorVar->SetBoolVector(valueArray);
	if (FAILED(res))
	{
		D3D11_INTERNAL_LogError(nullptr, "Failed to set Bool Vector", res);
		return;
	}
}

FX11API void FX11_Effect_Variable_SetVectori(ID3DX11EffectVariable* variable, const int* valueArray)
{
	ID3DX11EffectVectorVariable* vectorVar = variable->AsVector();
	HRESULT res;

	res = vectorVar->SetIntVector(valueArray);
	if (FAILED(res))
	{
		D3D11_INTERNAL_LogError(nullptr, "Failed to set Int Vector", res);
		return;
	}
}

FX11API void FX11_Effect_Variable_SetMatrix4f(ID3DX11EffectVariable* variable, const float* valueMatrix)
{
	ID3DX11EffectMatrixVariable* vectorVar = variable->AsMatrix();
	HRESULT res;

	res = vectorVar->SetMatrix(valueMatrix);
	if (FAILED(res))
	{
		D3D11_INTERNAL_LogError(nullptr, "Failed to set Matrix", res);
		return;
	}
}

FX11API void FX11_Effect_Variable_SetRawData(ID3DX11EffectVariable* variable, const void* pData, uint32_t byteOffset, uint32_t byteCount)
{
	HRESULT res;

	res = variable->SetRawValue(pData, byteOffset, byteCount);
	if (FAILED(res))
	{
		D3D11_INTERNAL_LogError(nullptr, "Failed to set Raw Data", res);
		return;
	}
}

FX11API void FX11_Effect_Variable_SetBool(ID3DX11EffectVariable* variable, bool value)
{
	ID3DX11EffectScalarVariable* scalarVar = variable->AsScalar();
	HRESULT res;

	res = scalarVar->SetBool(value);
	if (FAILED(res))
	{
		D3D11_INTERNAL_LogError(nullptr, "Failed to set Bool", res);
		return;
	}
}

FX11API void FX11_Effect_Variable_SetFloat(ID3DX11EffectVariable* variable, float value)
{
	ID3DX11EffectScalarVariable* scalarVar = variable->AsScalar();
	HRESULT res;

	res = scalarVar->SetFloat(value);
	if (FAILED(res))
	{
		D3D11_INTERNAL_LogError(nullptr, "Failed to set Float", res);
		return;
	}
}

FX11API void FX11_Effect_Variable_SetIntArray(ID3DX11EffectVariable* variable, const int* value, uint32_t offset, uint32_t length)
{
	ID3DX11EffectScalarVariable* scalarVar = variable->AsScalar();
	HRESULT res;
	res = scalarVar->SetIntArray(value, offset, length);
	if (FAILED(res))
	{
		D3D11_INTERNAL_LogError(nullptr, "Failed to set Integer Array", res);
		// FNA3D_LogInfo("Error: (%s)", "Failed to set Integer");
		return;
	}
}

FX11API void FX11_Effect_Variable_SetFloatArray(ID3DX11EffectVariable* variable, const float* value, uint32_t offset, uint32_t length)
{
	ID3DX11EffectScalarVariable* scalarVar = variable->AsScalar();
	HRESULT res;
	res = scalarVar->SetFloatArray(value, offset, length);
	if (FAILED(res))
	{
		D3D11_INTERNAL_LogError(nullptr, "Failed to set Float Array", res);
		// FNA3D_LogInfo("Error: (%s)", "Failed to set Integer");
		return;
	}
}

static HRESULT _FX11_Effect_GetDesc(ID3DX11Effect* effect, EffectReflectionData* result)
{
	HRESULT hr;
	hr = effect->GetDesc(&result->ShaderDesc);
	if (FAILED(hr))
	{
		return hr;
	}

	auto& shaderDesc = result->ShaderDesc;
	result->ConstantBufferCount = shaderDesc.ConstantBuffers;
	result->ConstantBufferDescs = (D3DX11_EFFECT_VARIABLE_DESC *)SDL_malloc(sizeof(D3DX11_EFFECT_VARIABLE_DESC) * shaderDesc.ConstantBuffers);
	for (UINT i = 0; i < shaderDesc.ConstantBuffers; ++i)
	{
		auto cbuffer = effect->GetConstantBufferByIndex(i);
		hr = cbuffer->GetDesc(&result->ConstantBufferDescs[i]);
	...
	result->Groups = (D3DX11_GROUP_DESC*)SDL_malloc(sizeof(D3DX11_GROUP_DESC) * shaderDesc.Groups);
	for (UINT i = 0; i < shaderDesc.Groups; ++i)
	{
		auto group = effect->GetGroupByIndex(i);
		hr = group->GetDesc(&result->Groups[i]);
		if (FAILED(hr))
		{
			return hr;
		}
	...
	result->Techniques = (D3DX11_TECHNIQUE_DESC*)SDL_malloc(sizeof(D3DX11_TECHNIQUE_DESC) * shaderDesc.Techniques);
	result->PTechniques = (ID3DX11EffectTechnique**)SDL_malloc(sizeof(ID3DX11EffectTechnique*) * shaderDesc.Techniques);
	result->Passes = (D3DX11_PASS_DESC**)SDL_malloc(sizeof(D3DX11_PASS_DESC*) * shaderDesc.Techniques); // new D3DX11_PASS_DESC * [shaderDesc->Techniques];
	result->PPasses = (ID3DX11EffectPass***)SDL_malloc(sizeof(ID3DX11EffectPass**) * shaderDesc.Techniques); // new ID3DX11EffectPass * *[shaderDesc->Techniques];
	for (UINT i = 0; i < shaderDesc.Techniques; ++i)
	{
		auto technique = effect->GetTechniqueByIndex(i);
		result->PTechniques[i] = technique;
		hr = technique->GetDesc(&result->Techniques[i]);
		...
		result->Passes[i] = (D3DX11_PASS_DESC*)SDL_malloc(sizeof(D3DX11_PASS_DESC) * result->Techniques[i].Passes); // new D3DX11_PASS_DESC[result->Techniques[i].Passes];
		result->PPasses[i] = (ID3DX11EffectPass**)SDL_malloc(sizeof(ID3DX11EffectPass*) * result->Techniques[i].Passes); // new ID3DX11EffectPass* [result->Techniques[i].Passes];
		for (UINT j = 0; j < result->Techniques[i].Passes; j++)
		{
			auto pass = technique->GetPassByIndex(j);
			result->PPasses[i][j] = pass;
			hr = pass->GetDesc(&result->Passes[i][j]);
	...
	result->Variables = (D3DX11_EFFECT_VARIABLE_DESC*)SDL_malloc(sizeof(D3DX11_EFFECT_VARIABLE_DESC) * shaderDesc.GlobalVariables);// new D3DX11_EFFECT_VARIABLE_DESC[shaderDesc->GlobalVariables];
	result->VariableTypes = (D3DX11_EFFECT_TYPE_DESC*)SDL_malloc(sizeof(D3DX11_EFFECT_TYPE_DESC) * shaderDesc.GlobalVariables); //new D3DX11_EFFECT_TYPE_DESC[shaderDesc->GlobalVariables];
	result->PVarialbes = (ID3DX11EffectVariable**)SDL_malloc(sizeof(ID3DX11EffectVariable*) * shaderDesc.GlobalVariables); //new ID3DX11EffectVariable*[shaderDesc->GlobalVariables];
	for (UINT i = 0; i < shaderDesc.GlobalVariables; ++i)
	{
		auto variable = effect->GetVariableByIndex(i);
		result->PVarialbes[i] = variable;
		hr = variable->GetDesc(&result->Variables[i]);
		if (FAILED(hr))
		{
			return hr;
		}

		hr = variable->GetType()->GetDesc(&result->VariableTypes[i]);
		if (FAILED(hr))
		{
			return hr;
		}
...
FX11API void FX11_Effect_CreateReflectionData(void* effect, EffectReflectionData* result)
{
	auto hr = _FX11_Effect_GetDesc(((D3D11Effect_New*)effect)->effect, result);
	if (FAILED(hr))
	{
		D3D11_INTERNAL_LogError(nullptr, "Shader Reflect Failed", hr);
	}
}

FX11API void FX11_Effect_ReleaseReflectionData(EffectReflectionData* data)
{
	SDL_free(data->ConstantBufferDescs);
	SDL_free(data->Groups);

	for (UINT i = 0; i < data->ShaderDesc.Techniques; ++i)
	{
		SDL_free(data->Passes[i]); 
		SDL_free(data->PPasses[i]);
	}

	SDL_free(data->Techniques);
	SDL_free(data->PTechniques);
	SDL_free(data->Passes);
	SDL_free(data->PPasses);
	SDL_free(data->Variables);
